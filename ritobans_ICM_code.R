...
data<-function(country,date_initial,date_final){
  ...
  data_country_cases = colSums((data_world_confirmed %>% filter(Country.Region == country))[, 5:ncol(data_world_confirmed)])
  ...
  data=cbind.data.frame(dates,I)
  return(data)
}
data=data("Mexico",date_initial,date_final)
colnames(data)=c("Date","Mexico")
rownames(data)=c(1:nrow(data))
dat<-data
o2d<-function(){
  i2o <- EuropeCovid$obs$deaths$i2o
  shape1 <- 5.807; scale1 <- 0.9; # infection to onset https://www.acpjournals.org/doi/10.7326/M20-0504
...
deaths <- epiobs(
  ...
  prior_intercept = rstanarm::normal(location=0.3,scale = 0.1),
...
w=w[c(1:nrow(data))]
args$data$week <- as.character(rep(w,times=1))
nrow(data)
